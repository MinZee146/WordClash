...
namespace AssetKits.ParticleImage
{
    ...
    public sealed class ParticleImage : MaskableGraphic
    {
        ...
        public ParticleImage GetMain()
        {
            if (transform.parent)
            {
                if (transform.parent.TryGetComponent<ParticleImage>(out var p))
                {
                    return p.GetMain();
                }
            }
        ...
        private void UpdateSheets()
        {
            ...
            {
                for (var i = textureTile.y - 1; i > -1; i--)
                {
                    for (var j = 0; j < textureTile.x; j++)
                    {
        ...
        {
            var numFrames = Mathf.FloorToInt(duration * 2 / 0.01f);
        ...
        private void Simulate(float deltaTime, bool prewarming = false)
        {
            ...
            if (isPlaying)
            {
                ...
                if (_emitterConstraintTransform && _emitterConstraintEnabled.enabled)
                {
                    ...
                    {
                        Vector3 canPos;
                        var viewportPos = mainCamera.WorldToViewportPoint(_emitterConstraintTransform.position);
