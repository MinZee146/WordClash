...
namespace AssetKits.ParticleImage {
    public class Particle
    {
        ...
        public void Simulate(float deltaTime)
        {
            ...
            //Apply color
            var c = _source.colorOverLifetime.Evaluate(_normalizedTime, _colorLerp);
            ...
            //Apply size
            var sol = _source.sizeOverLifetime.Evaluate(_normalizedTime, _sizeLerp);
            var sbs = _source.sizeBySpeed.Evaluate(normalizedSpeed.Remap(_source.sizeSpeedRange.from, _source.sizeSpeedRange.to, 0f, 1f), _sizeLerp);
            ...
            if (_source.trailsEnabled && _trailPoints.Count > 0)
            {
                ...
                if (_time < _lifetime)
                {
                    var tp = new TrailPoint(_position, _time);
                ...
                {
                    var pointDirection = _trailPoints[i].point;
                    var pointDistance = (i > 0) ? Vector3.Distance(_trailPoints[0].point, _trailPoints[i].point).Remap(0, trailLength, 1f, 0f) : 1f;
                    var pointWidth = _size.x * _source.trailWidth.Evaluate(pointDistance, _sizeLerp);
                    ...
                    var mid = _trailPoints[i].point; // the mid-point between start and end.
                    var perp = Vector2.Perpendicular(pointDirection.normalized); // vector of length 1 perpendicular to v.

                    var tc = _source.trailColorOverTrail.Evaluate(pointDistance, _colorLerp) *
