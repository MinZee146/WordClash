...
public class RewardManager : SingletonPersistent<RewardManager>
{
    ...
    {
        MocaLib.Instance.AnalyticsManager.LogEvent("buy_hints");
    ...
    public void RewardedAdCoins()
    {
        var dailyAdCount = PlayerPrefs.GetInt(GameConstants.PLAYER_PREFS_DAILY_AD_COUNT);
        if (dailyAdCount <= 0 || CurrencyManager.Instance.IsAnimating)
        {
            return;
        }

        MocaLib.Instance.AdManager.ShowRewardedAd((isSuccessful, adDuration) =>
       {
           if (isSuccessful)
           {
               ...
               dailyAdCount--;
               _dailyAdCount.text = dailyAdCount.ToString();

               PlayerPrefs.SetInt(GameConstants.PLAYER_PREFS_DAILY_AD_COUNT, dailyAdCount);
               CurrencyManager.Instance.CoinsAttract(RemoteConfigs.Instance.GameConfigs.CoinsPerAd, _dailyAdButton.transform.position);
               SetAdCoinsButton(PlayerPrefs.GetInt(GameConstants.PLAYER_PREFS_DAILY_AD_COUNT) > 0);
           }
       });
    }
