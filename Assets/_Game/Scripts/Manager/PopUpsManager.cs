...
public class PopUpsManager : Singleton<PopUpsManager>
{
    ...
    public void ToggleDoubleRewardPopUp(bool setActive, Action<TextMeshProUGUI, TextMeshProUGUI, Image> loadReward = null)
    {
#if UNITY_EDITOR
        DoubleRewardPopUp.Instance.IsRewardedAdReady = true;
#endif

        if (DoubleRewardPopUp.Instance.IsRewardedAdReady)
        {
            AudioManager.Instance.PlaySFX("ButtonClick");
            TogglePopUp(_doubleReward, setActive,
            onLoaded: (popUp) =>
            {
                popUp.transform.GetChild(4).GetComponent<Button>().onClick.AddListener(() =>
                {
                    ToggleDoubleRewardPopUp(false);
                });

                popUp.transform.GetChild(2).GetComponent<Button>().onClick.AddListener(() =>
                {
                    DoubleRewardPopUp.Instance.WatchAd();
                });

                var reward = popUp.transform.GetChild(3);
                var rewardName = reward.transform.GetChild(2).GetComponent<TextMeshProUGUI>();
                var rewardAmount = reward.transform.GetChild(3).GetComponent<TextMeshProUGUI>();
                var rewardImage = reward.transform.GetChild(1).GetComponent<Image>();

                loadReward?.Invoke(rewardName, rewardAmount, rewardImage);
            },
            onClosed: () =>
            {
                if (_currentRef == _gameOver)
                {
                    var totalMatchPlayed = PlayerPrefs.GetInt(GameConstants.PLAYER_PREFS_TOTAL_MATCH_PLAYED, 0);
                    if (totalMatchPlayed == RemoteConfigs.Instance.GameConfigs.RatingShowAtMatch - 1)
                    {
                        MocaLib.Instance.RatingManager.Show();
                    }

                    UIManager.Instance.SetButtonInGameOverActive(HomeButton.gameObject, ReplayButton.gameObject);
                }
            });
        }
        else
